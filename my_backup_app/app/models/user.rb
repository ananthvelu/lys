...
#       );
  ...
  def self.check_and_validate_new_user(newUser)
    ...
    @user = User.find(newUser["fbId"])
    if @user.length >0
      return false, @user
    else
        logger.info("no record found for this user")
        unless newUser["fbId"].nil && newUser["username"].nil then
          return true
        end  
        logger.info("fbID or username missing")
